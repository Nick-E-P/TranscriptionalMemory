function [R0] = GetR0R0coeff(X,x,yTOT,gamma)
%GETR0R0COEFF Summary of this function goes here
...
for i = 1:length(x)
    ...
    cov_E = cov(E_X(i,:),E_Y(i,:));
    E_var = mean([K1_store(i,:),K2_store(i,:)]);
    var_E = var([E_X(i,:),E_Y(i,:)]);
    theo_corr(i) = (E_cov + cov_E(1,2))/(E_var+var_E);
end

firstcells = yTOT(1,1:2:end);
secondcells = yTOT(1,2:2:end);
corrstart = corr(firstcells',secondcells');

R2 = corr((E_X(i,:))',(E_Y(i,:))');
R0 = (corrstart*(var_E+E_var)-R2*var_E)/E_var;
